...
Exemple (smoke test) :
    python src/build_aspect_labels.py --limit 50

Exemple (jeu complet, CPU-friendly) :
    python src/build_aspect_labels.py

Exemple (meilleure qualite NLI, plus lent) :
    python src/build_aspect_labels.py --zs-model facebook/bart-large-mnli
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--input",
        type=Path,
        default=ROOT / "data" / "raw" / "Baby_Products_echanillon.jsonl",
        help="JSONL d'avis bruts",
    )
    p.add_argument(
        "--output-flat",
        type=Path,
        default=ROOT / "data" / "raw" / "Baby_Products_aspect_labels_flat.jsonl",
        help="Sortie plate (1 ligne = phrase x aspect)",
    )
    p.add_argument(
        "--output-nested",
        type=Path,
        default=ROOT / "data" / "raw" / "Baby_Products_aspect_labels.jsonl",
